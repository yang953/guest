...
class ModelTest(TestCase):
    def setUp(self):
        Event.objects.create(id=1,name="oneplus3",status=True,limit=2000,address='shenzhen',start_time='2016-08-31 02:18:22')
        Guest.objects.create(id=1,event_id=1,realname='alen',phone='13711001101',email='alen@mail.com',sign=False)

    def test_event_models(self):
        result = Event.objects.get(name="oneplus3")
...
class IndexPageTest(TestCase):
    ...
    def test_index_page_renders_index_template(self):
        '''测试index视图'''
        response = self.client.get('/index/') # 请求index路径
        self.assertEqual(response.status_code,200) # 断言状态是否是200
        self.assertTemplateUsed(response,'index.html') # 断言是否用给定的是index。html模板响应
...
class LoginActionTest(TestCase):
    ...
    def test_login_action_username_password_null(self):
        ...
        self.assertIn(b"username or password error!",response.content) # 断言返回的HTML页面中是否包含"username or password error!"提示字符串
...
class EventManageTest(TestCase):
    ...
    def test_event_manage_success(self):
        '''测试发布会：xiaomi5'''
        # 因为event_manage和search_name两个视图函数被@login_required修饰，所以测试前必须要登录成功，并且需要构造登录用户的数据
...
class GusetManageTest(TestCase):
    '''嘉宾管理'''
    def setUp(self):
        User.objects.create_user('admin','admin@mail.com','admin123456')
